...
const interviewFormSchema = z.object({
  ...
  description: z.string()
    .min(10, "Please provide a detailed job description (minimum 10 characters)")
    .max(MAX_DESCRIPTION_CHARS, `Job description must be less than ${MAX_DESCRIPTION_CHARS} characters for optimal AI processing`),
...
const CreateInterviewForm = () => {
  ...
  const onSubmit = async (data: InterviewFormValues) => {
    ...
    if (data.description.length > MAX_DESCRIPTION_CHARS) {
      toast.error(`Job description exceeds ${MAX_DESCRIPTION_CHARS} characters. Please shorten it.`);
      return;
    }

    if (data.description.length < 10) {
      toast.error("Please provide a more detailed job description (minimum 10 characters).");
      return;
    }
    ...
    try {
      const response = await fetch("/api/vapi/generate", {
        ...
        body: JSON.stringify({
          ...data,
          description: data.description.substring(0, MAX_DESCRIPTION_CHARS)
        }),
      });
      ...
      if (result.success) {
        ...
        // SPECIFIC ERROR HANDLING FOR GEMINI QUOTA
        if (result.error?.includes("quota") || result.error?.includes("429") || result.error?.includes("Gemini")) {
          toast.error("⚠️ AI service temporarily unavailable. Please try again in a few hours or contact support.");
        } else {
          toast.error(result.error || "❌ Failed to create interview. Please try again.");
        }
      }
    } catch (error) {
    ...
    <div className="w-full min-h-screen bg-dark-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto w-full">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      // Auto-trim if exceeds 2500 characters
                      if (value.length > MAX_DESCRIPTION_CHARS) {
                        const trimmedValue = value.substring(0, MAX_DESCRIPTION_CHARS);
                        e.target.value = trimmedValue;
                        form.setValue("description", trimmedValue);

                        // Show toast notification
                        toast.warning(`Job description trimmed to ${MAX_DESCRIPTION_CHARS} characters for optimal AI processing`);
                      }

                      // Update character count
                      setCharCount(value.length > MAX_DESCRIPTION_CHARS ? MAX_DESCRIPTION_CHARS : value.length);
                    }}
                  />
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-semibold ${charCount > MAX_DESCRIPTION_CHARS * 0.9 ? "text-warning-100" : "text-light-400"
                        }`}>
                        {charCount > MAX_DESCRIPTION_CHARS * 0.9 ? "⚠️ Approaching limit" : "Characters remaining:"}
                      </span>
                      <span className={`text-sm font-bold ${charCount > MAX_DESCRIPTION_CHARS * 0.9 ? "text-warning-100" :
                        charCount > MAX_DESCRIPTION_CHARS * 0.8 ? "text-warning-100/80" : "text-success-100"
                        }`}>
                        {MAX_DESCRIPTION_CHARS - charCount}
                      </span>
                    </div>
                    <div className="text-sm text-light-400 font-semibold">
                  ...
                  <div className="w-full bg-dark-300 rounded-full h-1.5 mt-2">
                    <div
                      className={`h-1.5 rounded-full transition-all duration-300 ${charCount > MAX_DESCRIPTION_CHARS * 0.9 ? "bg-warning-100" :
                        charCount > MAX_DESCRIPTION_CHARS * 0.8 ? "bg-warning-100/80" : "bg-success-100"
                        }`}
                      style={{ width: `${(charCount / MAX_DESCRIPTION_CHARS) * 100}%` }}
                    ></div>
                  </div>
                  ...
                  <div className="flex items-start gap-2 mt-2">
                    <span className="text-warning-100 text-sm">💡</span>
                    <p className="text-sm text-light-400">
                      <strong>For best results:</strong> Keep job descriptions under 4000 characters.
                      The AI processes the entire text, so concise descriptions work best.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-dark-200/50 rounded-2xl p-6 border border-light-600/20">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-success-100 focus:ring-offset-2 ${form.watch("isPublic") ? 'bg-success-100' : 'bg-dark-300'
                        }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${form.watch("isPublic") ? 'translate-x-5' : 'translate-x-0'
                          }`}
                      />
                    </button>
                  </div>
        ...
        <div className="text-center mt-8 px-2">
          <p className="text-light-400 text-sm">
            💡 <strong>Pro Tip:</strong> Copy and paste the job description, but keep it under {MAX_DESCRIPTION_CHARS} characters for optimal AI processing.
